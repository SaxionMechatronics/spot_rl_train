...
import spot_rl_train.tasks.manager_based.spot_rl_locomotion.mdp as mdp
...
class SpotSceneCfg(InteractiveSceneCfg):
    ...
    # robots
    robot: ArticulationCfg = _replace(SPOT_CFG, prim_path="{ENV_REGEX_NS}/Robot")
    # sensors
...
class SpotRewardsCfg:
    ...
    gait = RewardTermCfg(
        func=mdp.GaitReward,  # type: ignore[arg-type]
        weight=10.0,
        params={
            "std": 0.1,
            "max_err": 0.2,
...
class CurriculumCfg:
    ...
    terrain_levels = CurrTerm(func=mdp.terrain_levels_vel)  # type: ignore[arg-type]


@configclass
class SpotRlLocomotionEnvCfg(ManagerBasedRLEnvCfg):
...
class SpotRlLocomotionEnvCfg_PLAY(SpotRlLocomotionEnvCfg):
